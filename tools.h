...
namespace tools {
	...
	*/
	...
	bool c_exists(std::vector<std::vector<unsigned> > const& pi, unsigned const& a_index, unsigned const& b_index, unsigned& c_index) {
		...
		for (unsigned j=b_index; j>a_index; --j){ //poruszamy sie po sciezce krytycznej
	...
	int carlier(unsigned const& n, std::vector< std::vector<unsigned> > & tasks, unsigned& UB){
		...
		if (!c_exists(pi, a_index, b_index, c_index)){
		...
		r_old = pi[c_index][1];
		pi[c_index][1] = std::max<unsigned>(pi[c_index][1], r_prim+p_prim);
		...
		q_old = pi[c_index][3];
		pi[c_index][3] = std::max<unsigned>(pi[c_index][3], q_prim+p_prim);
